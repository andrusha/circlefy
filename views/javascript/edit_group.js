...
function send_update(type){
	...
	if(type == 'you'){
		param += '&type='+type;
		param += '&gid='+ggid;
		param += '&gname='+document.getElementById('gname_input').value;
		param += '&focus='+document.getElementById('focus_input').value;
		param += '&descr='+document.getElementById('descr_input').value;
	}
       if (postText.readyState == 4 || postText.readyState == 0)
        {
         postText.open("POST", '../AJAX/edit_group.php', true);
...
function handleUpdate(imStatus,type){
   if (imStatus.readyState == 4) {
		...
		if(json_result != ''){
			document.getElementById('you_updated').innerHTML = 'Group info updated';
