...
function handleTap(imStatus,fid,state) {
		...
		if(state == 1){
                document.getElementById('tap_'+fid).innerHTML = '<span class="tap_box">Untap <img src="images/icons/connect.png" /></span>';
		document.getElementById('tap_'+fid).className += state;
                } else {
		document.getElementById('tap_'+fid).innerHTML = '<span class="tap_box">Tap <img src="images/icons/disconnect.png" /></span>';
                document.getElementById('tap_'+fid).className += state;
		}
	}
}
