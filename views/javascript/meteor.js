...
Meteor = {
	...
	connect: function() {
		...
		if (Meteor.mode=="xhrinteractive" || Meteor.mode=="iframe" || Meteor.mode=="serversent") {
			if (Meteor.mode == "iframe") {
				...
				Meteor.loadFrame(""+Meteor.host+((Meteor.port==80)?"":":"+Meteor.port)+"/stream.html");
			}
			clearTimeout(Meteor.pingtimer);
			Meteor.pingtimer = setTimeout(Meteor.pollmode, Meteor.pingtimeout);

		} else {
			Meteor.loadFrame(""+Meteor.host+((Meteor.port==80)?"":":"+Meteor.port)+"/poll.html");
	...
	getSubsUrl: function() {
		var surl = "" + Meteor.host + ((Meteor.port==80)?"":":"+Meteor.port) + "/push/" + Meteor.hostid + "/" + Meteor.mode;
