	...
	function not_done_actions($plugType){
		if ($this->step=='send_invites'){
			...
			if (count($this->contacts)==0)
				...
				{
				$odd=true;$counter=0;
				$contents .= "<table id='check_box_table'><td class='checkbox_col'><input type='checkbox' onChange='toggleAll(this)' name='toggle_all' title='Select/Deselect all' checked></td><td class='name_col'><b>Invite All</b></td>";
				foreach ($this->contacts as $email=>$name)
					{
					$counter++;
						$contents .= "\n<tr> <td class='checkbox_col'> <input name='check_{$counter}' value='{$counter}' type='checkbox' class='thCheckbox' checked><input type='hidden' name='email_{$counter}' value='{$email}'><input type='hidden' name='name_{$counter}' value='{$name}'></td> <td class='name_col'> {$name} </td> </tr>";
					$odd=!$odd;
					}
					$contents .= "</table><span id='oi_session'>$this->session</span>";
				}
			}
		}
	return $contents;	
